...
document context
Syntax: context
| Print context window, i.e. regs, stack, ds:esi and disassemble cs:eip in
| assem mode, or nearby source code in source mode.
end
...
document n
Syntax: n <NUM>
| Proceed by one step (instruction or line, depending on assem mode). Proceed
| through subroutine calls. If NUM is given, then repeat it NUM times or till
| program stops.
end
...
document s
Syntax: s <NUM>
| Proceed by one step, stepping into procedure calls.
...
define disablecolorprompt
    # Escape sequence at the end is a hack to make the trailing space print
    set extended-prompt gdb $ \[\e[0m\]
end
...
define enablecolorprompt
    set extended-prompt \[\e[1;34m\]gdb $ \[\e[0m\]
end
